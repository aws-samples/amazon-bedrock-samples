...
def build_converse_system_content():
    """Build system content for Converse API with cache checkpoint."""
    return [
        {"text": SYSTEM_PROMPT},
...
def test_converse_stream_system(client):
    """Test system prompt caching with ConverseStream API."""
    print("\n" + "=" * 60)
    print("Test 1: ConverseStream (system prompt)")
    print("=" * 60)
    ...
    for attempt, label in [(1, "first"), (2, "second")]:
        ...
        response = client.converse_stream(
            modelId=MODEL_ID,
            system=build_converse_system_content(),
            messages=[
                {
                    "role": "user",
...
def test_converse_system(client):
    """Test system prompt caching with Converse API."""
    print("\n" + "=" * 60)
    print("Test 2: Converse (system prompt)")
    print("=" * 60)
...
Examples:
  python test_system_prompt_caching.py
    ...
    results["ConverseStream"] = test_converse_stream_system(client)
    results["Converse"] = test_converse_system(client)
