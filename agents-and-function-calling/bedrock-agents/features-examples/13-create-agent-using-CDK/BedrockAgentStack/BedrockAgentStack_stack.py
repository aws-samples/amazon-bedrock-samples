...
class BedrockagentStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        s3Bucket = s3.Bucket(self, 'kbs3bucket', bucket_name=s3_bucket_name,
                             removal_policy=RemovalPolicy.DESTROY,
                             auto_delete_objects=True, 
                             event_bridge_enabled=True)
        bucket_deployment = s3_deploy.BucketDeployment(self, "Deploycontent",
                                sources=[s3_deploy.Source.asset("./dataset")],
                                destination_bucket=s3Bucket)

         # OpenSearch Serverless collection
        collection = os_serverless.CfnCollection(scope=self,
                                                 id='AgentCollection',
        ...
        network_policy_document = json.dumps([{'Rules': [{'Resource': [f'collection/{collection.name}'],
        ...
        base_lambda_policy = iam.ManagedPolicy.from_aws_managed_policy_name(managed_policy_name='service-role/AWSLambdaBasicExecutionRole')
        kb_lambda_role.add_to_policy(iam.PolicyStatement(sid='SyncKBPolicy', 
                                                         effect=iam.Effect.ALLOW,
                                                         resources=[knowledge_base.attr_knowledge_base_arn],
                                                         actions=['bedrock:StartIngestionJob']))
        kb_sync_lambda = lambda_.Function(scope=self,
                        id='SyncKB',
                        runtime=lambda_.Runtime.PYTHON_3_12,
        ...
        agent_role.add_to_policy(iam.PolicyStatement(sid='InvokeBedrockLambda',
                                                     effect=iam.Effect.ALLOW,
        ...
        action_group_function.add_to_role_policy(iam.PolicyStatement(sid="UpdateDynamoDB",
                                                     effect=iam.Effect.ALLOW,
        ...
        CfnOutput(scope=self, id='Knowedgebase_id', value=knowledge_base.attr_knowledge_base_id)
