    ...
    for _message in messages:
        if last_role == _message.get("role", ""):
            ...
            if "claude" in model_id:
                merged_messages[len(merged_messages) - 1].get("content", []).extend(_message.get("content"))
            elif "mistral" in model_id:
                # For mistral override
...
Let's first understand the question and generate the full plan within <plan></plan> tags.
Here is an example of how to generate the plan. You must always adhere to this example.

For example: Let's say there are two tools X() and Y().
The input parameter to X is A and the output is B. Similarly, input parameter to Y is B and output is C. Create a plan to solve the problem.

Output the equations in a numbered list within the <thinking> tags like below
<plan>
    <step_1> B=fn::X(A=a) </step_1>
    <step_2> C=fn::Y(B=b) </step_2>
    <step_3> return C </step_3>
</plan>
In each step just add the function call to be made, so that it is easily parsable.
For loops should be specified as <for expression="variable_name in variable">...</for> like below
<step_1>
    result = []
    <for expression="variable_name in variable">
        B=fn::X(A=a)
    </for>
</step_1>
"""
