...
class BedrockKnowledgeBase:
    ...
    def create_bedrock_execution_role_multi_ds(self, bucket_names=None, secrets_arns=None):
        ...
        try:
            existing_role = self.iam_client.get_role(RoleName=self.kb_execution_role_name)
            print(f"Using existing IAM role: {self.kb_execution_role_name}")
            return existing_role
        except self.iam_client.exceptions.NoSuchEntityException:
            # Role doesn't exist, continue with creation
            print(f"Creating new IAM role: {self.kb_execution_role_name}")
            pass
        ...
        assume_role_policy_document = {
            ...
            "Statement": [{
                ...
                "Condition": {
                    "StringEquals": {
                        "aws:SourceAccount": f"{self.account_number}"
                    },
                    "ArnLike": {
