...
description: >
  Diagnoses failing Amazon Bedrock Automated Reasoning (AR) tests and repairs policies using annotations.
  Classifies the failure by result type, applies the two-step (translate vs. validate) triage, and fixes
  issues via REFINE_POLICY annotations, RESOLVE_POLICY_AMBIGUITIES, or ITERATIVELY_REFINE_POLICY.

  Use this skill whenever the user wants to:
  - "Fix my Automated Reasoning policy" / "my AR test is failing"
  - Debug an unexpected IMPOSSIBLE, TRANSLATION_AMBIGUOUS, INVALID, SATISFIABLE, or VALID result
  - Resolve conflicting rules, unused/overlapping variables, or bare assertions
  - Apply annotations (addRule, updateVariable, addRuleFromNaturalLanguage, etc.) or refine a policy
  Trigger when ar-policy-tester reports failures, or when validation returns surprising results.
license: Apache-2.0
---
