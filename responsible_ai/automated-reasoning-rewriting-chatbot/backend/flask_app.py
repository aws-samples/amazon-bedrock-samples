...
from backend.models.thread import IterationType, ClarificationIterationData, ThreadStatus
...
# ============================================================================
# Error Handlers (registered on app, not blueprint)
# ============================================================================
...
@api_bp.route('/config/policies', methods=['GET'])
def get_policies():
...
@api_bp.route('/policy/<path:policy_arn>/test-cases', methods=['GET'])
...
@api_bp.route('/thread/<thread_id>/answer', methods=['POST'])
def submit_answers(thread_id: str):
    ...
    try:
        # In testing mode, process synchronously for deterministic tests
        if current_app.config.get('TESTING'):
            logger.info(f"Testing mode: resuming thread {thread_id} synchronously")
...
def register_routes(app: Flask) -> None:
    """
    Register all API routes via blueprints.
