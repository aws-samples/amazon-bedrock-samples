...
def play(filename):
    ...
    html += '<video width=1000 controls loop><source src="%s" type="video/mp4"></video>' % src 
    return HTML(html)
...
def display_text_and_unique_image(response):
    # Print text response
    print("Text Response:")
    print(response['output']['text'])
    print("\n-------------------\n")
    
    # Keep track of seen S3 URIs
    seen_s3_uris = set()
    ...
    if 'citations' in response and len(response['citations']) > 0:
        for citation in response['citations']:
            if 'retrievedReferences' in citation:
                for ref in citation['retrievedReferences']:
                    if ref['content']['type'] == 'IMAGE':
                        # Check S3 URI
                        s3_uri = ref['metadata'].get('x-amz-bedrock-kb-source-uri', '')
                        
                        # Only process if we haven't seen this image before
                        if s3_uri not in seen_s3_uris:
                            seen_s3_uris.add(s3_uri)
                            
                            base64_data = ref['content']['byteContent']
