...
try:
    ...
    action_group_id = next(
        (agent['actionGroupId'] for agent in list_action_group if agent['actionGroupName'] == action_group_name), None)
    ...
    agentAliasId = next((agent['agentAliasId'] for agent in response['agentAliasSummaries'] if
                         agent['agentAliasName'] == agent_alias_name), None)
    agentAliasId
except:
    pass
