...
try:
    ...
    logger.info(f"Successfully created Bedrock runtime client using profile: {profile}")
except Exception as e:
    logger.error(f"Failed to create Bedrock runtime client: {str(e)}")
    raise
...
def run_multiple_calls(n=2):
    ...
    for i in range(n):
        ...
        try:
            ...
            shortened_response = response_text[:150] + "..." if len(response_text) > 150 else response_text
            ...
            logger.error(f"Failed on request {i+1}: {str(e)}")
            results.append({
                'metadata': metadata,
                'prompt': prompt,
                'error': str(e),
