...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;i. Under **Provide knowledge base details**, enter a _Knowledge base name_ and optional _Description_, leaving all other default settings.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ii. Under **Set up data source**, enter a _Data source name_ then choose _Browse S3_ and select the knowledge base data source S3 bucket you deployed in the preceding deployment step (e.g., \<YOUR-STACK-NAME>-bedrock-kb).

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;iii. Under **Configure vector store**, leave the default settings. An [OpenSearch Serverless vector store](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/serverless-vector-search.html) will be created for you. This vector store is where the knowledge base pre-processing embeddings are stored and later used for semantic similarity search between queries and data source text.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;iv. Under **Review and create**, confirm your configuration settings then select **Create knowledge base**.
...
&nbsp;&nbsp;&nbsp;b. Once your knowledge base is created, a green "created successfully" banner will display with the option to sync your data source. Select **Sync** to initiate the data source sync:
...
&nbsp;&nbsp;&nbsp;c. Navigate to the [Knowledge Base Console](https://us-east-1.console.aws.amazon.com/bedrock/home?region=us-east-1#/knowledge-bases), select the knowledge base you just created, then note the **Knowledge base ID** under Knowledge base overview:
...
&nbsp;&nbsp;&nbsp;d. With your knowledge base still selected in the Knowledge Base Console, select your knowledge base data source listed under **Data source**, then note the **Data source ID** under _Data source overview_:
...
❗ Knowledge base ID and Data source ID will be used as environment variables in the later [Deploy CloudFormation Stack to Emulate Existing Customer Resources](#deploy-cloudFormation-stack-to-emulate-existing-customer-resources) deployment step. Please set those environment variables now:
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;i. Enter an _Agent name_ and optional _Description_, leaving all other default settings.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ii. Select **Anthropic Claude V2** for _Model details_ and specify the below _Instructions for the Agent_, then select **Next**:

```
You are an agent designed to help with processing insurance claims and managing pending paper work
```
&nbsp;&nbsp;&nbsp;iii. Add the following three action groups then select **Next**:
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;b. Under **Select Lambda function**, choose _\<YOUR-STACK-NAME>-CreateClaimFunction_.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;c. Under **Select API schema**, choose _Browse S3_, pick the bucket created during the preceding deployment step (e.g, \<YOUR-STACK-NAME>-customer-resources), then select _agent/api-schema/create_claim.json_:
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;b. Under **Select Lambda function**, choose _\<YOUR-STACK-NAME>-GatherEvidenceFunction_.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;c. Under **Select API schema**, choose _Browse S3_, pick the bucket created during the preceding deployment step, then select _agent/api-schema/gather_evidence.json_.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;b. Under **Select Lambda function**, choose _\<YOUR-STACK-NAME>-SendReminderFunction_.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;c. Under **Select API schema**, choose _Browse S3_, pick the bucket created during the preceding deployment step, then select _agent/api-schema/send_reminder.json_.

&nbsp;&nbsp;&nbsp;iv. Add Knowledge base:

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;a. Under **Select knowledge base**, select the knowledge base you created in the preceding deployment step (e.g., claims-knowledge-base).

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;b. Under **Knowledge base instructions for Agent**, enter the following then select **Next**:
```
...
&nbsp;&nbsp;&nbsp;v. Verify your configuration settings then select **Create agent**. 
